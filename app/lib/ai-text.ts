const phraseReplacements: [RegExp, string][] = [
  ...
  [/метод\s+Pomodoro/gi, "режим 25/5"],
  [/во\s+время\s+peak\s+hours/gi, "в лучшее время дня"],
  [/Улучшите\s+фокус\s+в\s+peak\s+hours/gi, "Улучшите фокус в лучшее время дня"],
  [/Уменьшите\s+интерр?упц(?:ии|ий|ия|ию)/gi, "Уменьшите прерывания"],
  [/Уменьшите\s+разрывы/gi, "Уменьшите прерывания"],
];

const wordReplacements: [RegExp, string][] = [
  [/\bPomodoro\b/gi, "25/5"],
  [/\bpeak\s+hours\b/gi, "лучшее время дня"],
  [/\bpeak\b/gi, "лучшее"],
  [/\bhours\b/gi, "часы"],
  [/\binterruptions?\b/gi, "прерывания"],
...
const cleanupReplacements: [RegExp, string][] = [
  ...
  [/интерр?упц(?:ии|ий|ия|ию|иями|иях)?/gi, "прерывания"],
  [/интерр?у(?:кц|ч|щ|ц)(?:ии|ий|ия|ию|иями|иях)?/gi, "прерывания"],
  [/ин\p{L}{0,6}ру\p{L}{0,8}/giu, "прерывания"],
  [/интерр?у\p{L}{1,8}/giu, "прерывания"],
  [/количество\s+прерывания/gi, "количество прерываний"],
  [/среднее\s+количество\s+прерывания/gi, "среднее количество прерываний"],
  [/уменьшить\s+количество\s+прерывания/gi, "уменьшить количество прерываний"],
  [/уменьшите\s+количество\s+прерывания/gi, "уменьшите количество прерываний"],
  [/уменьшите\s+разрывы/gi, "уменьшите прерывания"],
  [/количество\s+разрывов/gi, "количество прерываний"],
  [/режим\s+25\/5\s+для/gi, "режим 25/5 для"],
  [/метод\s+25\/5/gi, "режим 25/5"],
  [/время\s+лучшее\s+часы/gi, "лучшее время дня"],
  [/в\s+лучшее\s+часы/gi, "в лучшее время дня"],
  [/в\s+лучшее\s+время\s+дня/gi, "в лучшее время дня"],
  [/(^|[.!?]\s*)прерывания:/giu, "$1Прерывания:"],
];
